...
impl Joypad {
    ...
    pub fn down(&mut self, val: bool) {
        if val && !self.down { self.interrupt = true; }
        self.down = val;
    }

    pub fn left(&mut self, val: bool) {
        if val && !self.left { self.interrupt = true; }
        self.left = val;
    }

    pub fn right(&mut self, val: bool) {
        if val && !self.right { self.interrupt = true; }
        self.right = val;
    }

    pub fn a(&mut self, val: bool) {
        if val && !self.a { self.interrupt = true; }
        self.a = val;
    }

    pub fn b(&mut self, val: bool) {
        if val && !self.b { self.interrupt = true; }
        self.b = val;
    }

    pub fn select(&mut self, val: bool) {
        if val && !self.select { self.interrupt = true; }
        self.select = val;
    }

    pub fn start(&mut self, val: bool) {
        if val && !self.start { self.interrupt = true; }
        self.start = val;
    }

    pub fn up(&mut self, val: bool) {
        if val && !self.up { self.interrupt = true; }
        self.up = val;
    }
