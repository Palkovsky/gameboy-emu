...
pub enum TimerMode { FQ_4096HZ, FQ_16384HZ, FQ_65536HZ, FQ_262144HZ }
...
impl <T: BankController>Clocked<T> for Timer {
    ...
    fn step(&mut self, mmu: &mut MMU<T>) { 
        ...
        let mut check_ticks = |steps: u64| {
            ...
            if self.tima_cycle % steps != 0 { return }
...
impl Timer {
    ...
    /* Called when something from outside writes to DIV. */
    pub fn reset_internal_tima(&mut self) { self.tima_cycle = 1; }
    ...
    pub fn DIV<T: BankController>(mmu: &mut MMU<T>) -> u8 { mmu.read(ioregs::DIV) }
    pub fn TIMA<T: BankController>(mmu: &mut MMU<T>) -> u8 { mmu.read(ioregs::TIMA) }
    pub fn TMA<T: BankController>(mmu: &mut MMU<T>) -> u8 { mmu.read(ioregs::TMA) }

    fn _DIV<T: BankController>(mmu: &mut MMU<T>, val: u8) { mmu.write(ioregs::DIV, val); }
...
*/
    ...
    pub fn _MODE<T: BankController>(mmu: &mut MMU<T>, mode: TimerMode) { 
        match mode {
            TimerMode::FQ_16384HZ => // 11
                { mmu.set_bit(ioregs::TAC, 1, true); mmu.set_bit(ioregs::TAC, 0, true); },      
            TimerMode::FQ_65536HZ => // 10
                { mmu.set_bit(ioregs::TAC, 1, true); mmu.set_bit(ioregs::TAC, 0, false); },   
            TimerMode::FQ_262144HZ => // 01
                { mmu.set_bit(ioregs::TAC, 1, false); mmu.set_bit(ioregs::TAC, 0, true); },  
            TimerMode::FQ_4096HZ =>   // 00
                { mmu.set_bit(ioregs::TAC, 1, false); mmu.set_bit(ioregs::TAC, 0, false); },  
        } 
    }
}