...
impl <T: BankController>Clocked<T> for DMA {
    ...
    fn step(&mut self, mmu: &mut MMU<T>) { 
        ...
        for i in 0..TRANSFER_SIZE { self.buff[i] = mmu.read(addr + i as u16); }
        let dest = &mut mmu.oam[..];
        for i in 0..TRANSFER_SIZE { dest[i] = self.buff[i]; }
...
impl DMA {
    pub fn new() -> Self { Self { active: false, buff: [0; TRANSFER_SIZE]} }
    pub fn start(&mut self) { self.active = true; }
    pub fn active(&self) -> bool { self.active }
    fn FROM(mmu: &mut MMU<impl BankController>) -> u16 { (mmu.read(ioregs::DMA) as u16) << 8 }
}
